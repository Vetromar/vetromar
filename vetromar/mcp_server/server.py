...
@mcp.tool()
def get_episode(episode_id: str, include_raw: bool = False) -> dict:
    ...
    return views.episode_detail(_get_store(), episode_id, include_raw=include_raw)
...
@mcp.tool()
def units_by_entity(entity_id: str) -> list[dict]:
...
@mcp.tool()
def current_state(entity_id: str | None = None) -> dict:
    ...
    on one entity."""
    return views.current_state(_get_store(), entity_id)
...
@mcp.tool()
def add_units(episode_id: str, units: list[dict], agent: str | None = None) -> list[dict]:
...
@mcp.tool()
def link_units(from_id: str, to_id: str, kind: str = "related") -> dict:
...
@mcp.tool()
def supersede_unit(old_id: str, new_id: str) -> dict:
